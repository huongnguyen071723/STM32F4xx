...
TIM_HandleTypeDef htime6;/*! the variable to handle the timer 6(basic timer)*/
...
int main() {
    ...
    if(HAL_TIM_Base_Start_IT(&htime6) != HAL_OK){ /*! Start count with interrupt mode*/
       Error_handler();
    }
...
/**
 * @fn      void TIM6_Init()
 * @brief   Initialization high level for the TIM6 peripheral
 * @var     void
 * @retval  void
 */
void TIM6_Init() {
    htime6.Instance = TIM6;/*The address of timer 6*/
    htime6.Init.Prescaler = 0xFFF; /*the prescaler for clock of timer 6*/
    htime6.Init.CounterMode = TIM_COUNTERMODE_UP;/*Default: the counter mode of timer 6 is TIM_COUNTERMODE_UP*/
    htime6.Init.Period = 2000;
    htime6.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;

    /*Initialized the timer 6*/
